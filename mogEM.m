...
vr = std(x,[],2).^2;
%use K means to update the Means
mu = kmeans(x,K,5);
...
for i=1:iters
  ...
  title('Log-probability of data versus # iterations of EM with Kmeans');
  ...
  if plotFlag     % Plot the data and Gaussians
    ...
    hold on;
    for k=1:K
      plotEllipse(mu(1,k),mu(2,k),vary(1,k),vary(2,k),0,ellColor(k));
    end;
    for t=1:T  plot(x(1,t),x(2,t),char(plotString(mxi(t)))); end
    axis equal;
  end;
